...
{
    public partial class frmYcEdit : Common.frmBase
    {
        public frmYcEdit()
        {
            InitializeComponent();
        ...
        private bool BindData()
        {
            try
            {
                this.myDataGridView1.DataSource = Common.CommonDAL.DoSqlCommandBasic.GetDateTable("SELECT * FROM NanJingZB_YaCha order by MyIndex", true);
            }
            catch(Exception ex)
            {
        ...
        private bool Save()
        {
            ...
            try
            {
                Common.CommonDAL.DoSqlCommandBasic.SaveTable(dt, "NanJingZB_YaCha");
            }
            catch(Exception ex)
            {
                this.ShowMsg(ex.Message);
