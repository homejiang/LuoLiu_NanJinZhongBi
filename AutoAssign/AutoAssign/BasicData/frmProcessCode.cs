...
{
    public partial class frmProcessCode : Common.frmBase
    {
        ...
        private bool BindData()
        {
            ...
            try
            {
                dt = Common.CommonDAL.DoSqlCommandBasic.GetDateTable("select * from V_JC_Process");
            }
            catch (Exception ex)
            {
        ...
        private void btAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                this.ShowMsg("请输入工序代码！");
                return;
            }
            DataTable dt;
            try
            {
                dt = Common.CommonDAL.DoSqlCommandBasic.GetDateTable(string.Format("SELECT COUNT(*) FROM JC_Process WHERE Code='{0}'", this.tbCode.Text.Replace("'","''")));
            }
            catch (Exception ex)
            {
                wErrorMessage.ShowErrorDialog1(this, ex, "校验工序代码是否重号");
                return;
            }
            if(int.Parse(dt.Rows[0][0].ToString())>0)
            {
                this.ShowMsg("工序编码已经存在！");
                return;
            }
            string strSql = string.Format("INSERT INTO JC_Process(Code) VALUES('{0}')", this.tbCode.Text.Replace("'", "''"));
            try
            {
        ...
        private void tsbDel_Click(object sender, EventArgs e)
        {
            ...
            foreach (int row in list)
            {
                ...
                string strSql = string.Format("delete from JC_Process where code='{0}'", sCode.Replace("'", "''"));
                try
                {
                    Common.CommonDAL.DoSqlCommandBasic.DoSql(strSql);
                }
        ...
        private void tsbOn_Click(object sender, EventArgs e)
        {
            ...
            {
                string sCode = dt.DefaultView[row].Row["Code"].ToString();
                string strSql = string.Format("update JC_Process set Terminated=0 where code='{0}'", sCode.Replace("'", "''"));
                try
                {
                    Common.CommonDAL.DoSqlCommandBasic.DoSql(strSql);
                }
        ...
        private void tsbOff_Click(object sender, EventArgs e)
        {
            ...
            {
                string sCode = dt.DefaultView[row].Row["Code"].ToString();
                string strSql = string.Format("update JC_Process set Terminated=1 where code='{0}'", sCode.Replace("'", "''"));
                try
                {
                    Common.CommonDAL.DoSqlCommandBasic.DoSql(strSql);
                }
