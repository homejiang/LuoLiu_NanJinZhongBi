...
namespace LuoLiuMESPrinter
{
    public partial class frmStationConfig : Common.frmBase
    {
        ...
        {
            if (!this.BindProcess(JpsConfig.ProcessCode))
                this._ProcessCode = string.Empty;
            else this._ProcessCode = JpsConfig.ProcessCode;
        ...
        private void btTrue_Click(object sender, EventArgs e)
        {
            ...
            JpsConfig.ProcessCode = this._ProcessCode;
            JpsConfig.StationCode = this._StationCode;
            JpsConfig.MacCode = this._MacCode;


            JpsConfig.ProcessName = this.tbProcess.Text;
            JpsConfig.StationName = this.tbStation.Text;
            JpsConfig.MacName = this.tbMac.Text;
            //写入配置文件呢
            Common.CommonFuns.ConfigINI.INIFileName = "Server.ini";
            try
            {
                Common.CommonFuns.ConfigINI.SetValue("Station", "ProcessCode", JpsConfig.ProcessCode);
                Common.CommonFuns.ConfigINI.SetValue("Station", "StationCode", JpsConfig.StationCode);
                Common.CommonFuns.ConfigINI.SetValue("Station", "MacCode", JpsConfig.MacCode);
            }
            catch (Exception ex)
            {
