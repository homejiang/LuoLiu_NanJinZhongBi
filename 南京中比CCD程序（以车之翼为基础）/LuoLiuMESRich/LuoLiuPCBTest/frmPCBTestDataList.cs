...
{
    public partial class frmPCBTestDataList : Common.frmBaseList
    {
        public frmPCBTestDataList()
        {
            InitializeComponent();
        }
        #region  重写父类函数
        ...
        private bool PerInit()
        {
            ...
            item.CanInput = true;
            item.DropdownItemsLoaded = true;
            item.StringFormat = "LotNumber like '%{0}%'";
            item.Value = 3;
            listSearchItem.Add(item);
            item = new Common.MyEntity.SearchLabelItem();
            item.TitleName = "测试人员";
            ...
            #endregion
            #region 绑定列表字段
            this.ShowDataGridViewSetting_BindColumn(this.dgvList, Common.MyEnums.Modules.PCBManage);
            #endregion
        ...
        private bool SetCurrentMacInfo()
        {
            ...
            else
            {
                ...
                try
                {
                    dt = Common.CommonDAL.DoSqlCommand.GetDateTable(string.Format("SELECT MacName FROM JC_ProcessMacs WHERE Code='{0}'", PCBConfig.MacCode.Replace("'", "''")), false);
                }
                catch (Exception ex)
                {
                    wErrorMessage.ShowErrorDialog(this, ex);
                ...
                if (dt.Rows.Count == 0)
                {
                    this.ShowMsg("系统设置的默认机台代码“" + PCBConfig.MacName + "”不存在或已经被删除。");
                    return false;
                }
        ...
        {
            string strSql;
            strSql = "SELECT * FROM [V_PCB_TestData_Cgy] WHERE 1=1";
            //设置生产机台号搜索
            if (PCBConfig.MacName.Length > 0)
                strSql += " AND (ISNULL(MacCode,'')='' OR MacCode='" + PCBConfig.MacCode.Replace("'", "''") + "')";
            //设置时间搜索
            strSql += string.Format(" AND EndTime>='{0} {1}'", this.BarSearchDateTimeStart.Value.ToString("yyyy-MM-dd"), this.BarSearchTimeStart.Value.ToString("HH:mm"));
            if (this.BarSearchDateTimeEnd.Checked)
            {
                strSql += string.Format(" AND EndTime<='{0} {1}'", this.BarSearchDateTimeEnd.Value.ToString("yyyy-MM-dd"), this.BarSearchTimeEnd.Value.ToString("HH:mm"));
            }
            //设置commbox搜索条件
            ...
            strSql += " ORDER BY EndTime DESC ";//按检测完成时间排序
            DataTable dt = null;
            try
            {
        ...
        private void OpenUpdateForm(int iRowIndex, DataTable dtList)
        {
            ...
            string strPcbCode = dtList.DefaultView[iRowIndex].Row["PcbCode"].ToString();
            if (strGuid.Length == 0) return;
            frmPCBTestDataEdit frm = new frmPCBTestDataEdit();
            frm.FormParent = this;

            //校验权限
            List<Common.MyEnums.OperatePower> listPower = this.GetOperatePower(Common.MyEnums.Modules.PCBManage, strPcbCode);
            if (listPower.Count == 0)
            {
                this.ShowMsg(string.Format("您没有权限打开保护板检测单“{0}”，因为你没有它的任何权限。", strPcbCode));
                return;
            }
            if (listPower.Contains(Common.MyEnums.OperatePower.Eidt))
            {
                frm.FormState = Common.MyEnums.FormStates.Edit;
                frm.Text = "保护板检测记录" + strPcbCode;
            ...
            else
            {
                ...
                frm.Text = string.Format("保护板检测记录{0}（只读）", strPcbCode);
                frm.PrimaryValue = strGuid;
                frm.TopMost = true;
        ...
        #endregion
        #region 窗体加载
        private void frmPCBTestDataList_Load(object sender, EventArgs e)
        {
