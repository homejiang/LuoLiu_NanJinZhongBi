...
namespace LuoLiuPCBTest
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            #endregion
            if (Common.CommonDAL.MainProgramName == "" || string.Compare(Common.CommonDAL.MainProgramName, "LuoLiuPCBTest.exe", true) == 0)
        ...
        static void StartPro()
        {
            ...
            PCBConfig cfg = new PCBConfig();
            if (!cfg.ReadConfig())
            {
                return;
            }
            if (PCBConfig.ProcessCode.Length == 0 || PCBConfig.StationCode.Length == 0 || PCBConfig.MacCode.Length == 0)
            {
            ...
            frmlogin1.Tag = "LuoLiuPCBTest.exe-" + Common.CommonDAL.StartArg;
            Application.Run(frmlogin1);
            if (frmlogin1.DialogResult != DialogResult.OK)
                return;
            if (!Common.CommonFuns.OneInstance.IsFirst("hangzhouchenhenLuoliuMesPrinterDevelopeBychenguiyang"))
            {
                MessageBox.Show("程序已经打开！");
                return;
            }
            LuoLiuPCBTest.frmPCBTestData frmAp = new LuoLiuPCBTest.frmPCBTestData();
            Application.Run(frmAp);
        }
    }
