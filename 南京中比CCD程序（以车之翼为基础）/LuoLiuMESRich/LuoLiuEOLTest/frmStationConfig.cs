...
namespace LuoLiuEOLTest
{
    public partial class frmStationConfig : Common.frmBase
    {
        ...
        {
            if (!this.BindProcess(LuoLiuEOLTest.EOLConfig.ProcessCode))
                this._ProcessCode = string.Empty;
            else this._ProcessCode = LuoLiuEOLTest.EOLConfig.ProcessCode;

            if (!this.BindStation(LuoLiuEOLTest.EOLConfig.StationCode))
                this._StationCode = string.Empty;
            else this._StationCode = LuoLiuEOLTest.EOLConfig.StationCode;

            if (!this.BindMac(LuoLiuEOLTest.EOLConfig.MacCode))
                this._MacCode = string.Empty;
            else this._MacCode = LuoLiuEOLTest.EOLConfig.MacCode;
        }
        ...
        private void btTrue_Click(object sender, EventArgs e)
        {
            ...
            LuoLiuEOLTest.EOLConfig.ProcessCode = this._ProcessCode;
            LuoLiuEOLTest.EOLConfig.StationCode = this._StationCode;
            LuoLiuEOLTest.EOLConfig.MacCode = this._MacCode;


            LuoLiuEOLTest.EOLConfig.ProcessName = this.tbProcess.Text;
            LuoLiuEOLTest.EOLConfig.StationName = this.tbStation.Text;
            LuoLiuEOLTest.EOLConfig.MacName = this.tbMac.Text;
            //写入配置文件呢
            Common.CommonFuns.ConfigINI.INIFileName = "Config.ini";
            try
            {
                Common.CommonFuns.ConfigINI.SetValue("Station", "ProcessCode", LuoLiuEOLTest.EOLConfig.ProcessCode);
                Common.CommonFuns.ConfigINI.SetValue("Station", "StationCode", LuoLiuEOLTest.EOLConfig.StationCode);
                Common.CommonFuns.ConfigINI.SetValue("Station", "MacCode", LuoLiuEOLTest.EOLConfig.MacCode);
            }
            catch (Exception ex)
            {
