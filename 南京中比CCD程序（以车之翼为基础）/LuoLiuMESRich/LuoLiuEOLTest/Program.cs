...
namespace LuoLiuEOLTest
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            #endregion
            if (Common.CommonDAL.MainProgramName == "" || string.Compare(Common.CommonDAL.MainProgramName, "LuoLiuEOLTest.exe", true) == 0)
        ...
        {
            if (!Common.CommonFuns.OneInstance.IsFirst("hangzhouchenhenLuoLiuEOLTestDevelopeBychenguiyang"))
            {
                MessageBox.Show("程序已经打开！");
                return;
            }
            EOLConfig cfg = new EOLConfig();
            if (!cfg.ReadConfig())
            {
                return;
            }
            if (EOLConfig.ProcessCode.Length == 0 || EOLConfig.StationCode.Length == 0 || EOLConfig.MacCode.Length == 0)
            {
            ...
            frmlogin1.Tag = "LuoLiuEOLTest.exe-" + Common.CommonDAL.StartArg;
            Application.Run(frmlogin1);
            if (frmlogin1.DialogResult != DialogResult.OK)
                return;
            Application.Run(new frmEOLTestData());
        }
    }
    #region 读取版本信息
