...
namespace LuoLiuCCD
{
    public partial class frmStationConfig : Common.frmBase
    {
        ...
        {
            if (!this.BindProcess(LuoLiuCCD.CcdConfig.ProcessCode))
                this._ProcessCode = string.Empty;
            else this._ProcessCode = LuoLiuCCD.CcdConfig.ProcessCode;

            if (!this.BindStation(LuoLiuCCD.CcdConfig.StationCode))
                this._StationCode = string.Empty;
            else this._StationCode = LuoLiuCCD.CcdConfig.StationCode;

            if (!this.BindMac(LuoLiuCCD.CcdConfig.MacCode))
                this._MacCode = string.Empty;
            else this._MacCode = LuoLiuCCD.CcdConfig.MacCode;
        }
        ...
        private void btTrue_Click(object sender, EventArgs e)
        {
            ...
            LuoLiuCCD.CcdConfig.ProcessCode = this._ProcessCode;
            LuoLiuCCD.CcdConfig.StationCode = this._StationCode;
            LuoLiuCCD.CcdConfig.MacCode = this._MacCode;


            LuoLiuCCD.CcdConfig.ProcessName = this.tbProcess.Text;
            LuoLiuCCD.CcdConfig.StationName = this.tbStation.Text;
            LuoLiuCCD.CcdConfig.MacName = this.tbMac.Text;
            //写入配置文件呢
            Common.CommonFuns.ConfigINI.INIFileName = "Config.ini";
            try
            {
                Common.CommonFuns.ConfigINI.SetValue("Station", "ProcessCode", LuoLiuCCD.CcdConfig.ProcessCode);
                Common.CommonFuns.ConfigINI.SetValue("Station", "StationCode", LuoLiuCCD.CcdConfig.StationCode);
                Common.CommonFuns.ConfigINI.SetValue("Station", "MacCode", LuoLiuCCD.CcdConfig.MacCode);
            }
            catch (Exception ex)
            {
