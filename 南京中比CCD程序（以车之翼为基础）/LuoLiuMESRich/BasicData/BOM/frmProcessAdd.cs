...
{
    public partial class frmProcessAdd : BOM.frmBOM
    {
        string _BomGuid = string.Empty;
        long _GuidBehind = -1;
        public frmProcessAdd(string sBOMGuid, long lGuidBehind)
        {
            InitializeComponent();
        ...
        private bool BindData()
        {
            ...
            try
            {
                dt = Common.CommonDAL.DoSqlCommand.GetDateTable(string.Format(@"select code,ProcessName from JC_Process A LEFT JOIN BOM_Product_Process B ON B.ProcessCode=A.Code AND B.ProGuid='{0}'
                WHERE B.ID IS NULL ORDER BY A.SortID ASC", _BomGuid.Replace("'", "''")));
            }
            catch (Exception ex)
            {
