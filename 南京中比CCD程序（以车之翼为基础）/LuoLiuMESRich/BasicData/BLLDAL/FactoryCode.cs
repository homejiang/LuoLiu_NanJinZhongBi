    ...
    public class FactoryCode
    {
        ...
        public void Save(DataSet ds)
        {
            ...
            try
            {
                ...
                DataRow[] drs;
                #region 新增数据
                drs = ds.Tables["JC_FactoryCode"].Select("", "", DataViewRowState.Added);
                strSql = @"INSERT INTO JC_FactoryCode (Code,CodeName,AnotherName,Remark,Terminated) 
                ...
                #endregion
                #region 更新数据
                drs = ds.Tables["JC_FactoryCode"].Select("", "", DataViewRowState.ModifiedCurrent);
                strSql = @"UPDATE JC_FactoryCode SET Code=@Code,CodeName=@CodeName,AnotherName=@AnotherName,Remark=@Remark,Terminated=@Terminated  WHERE Code=@OriginalCode";
                foreach (DataRow dr in drs)
                {
                    sqlCom = new SqlCommand(strSql, sqlConn, sqlTrain);
                    sqlParam = new SqlParameter();
                ...
                #endregion
                #region 删除数据
                drs = ds.Tables["JC_FactoryCode"].Select("", "", DataViewRowState.Deleted);
                strSql = @"DELETE FROM JC_FactoryCode WHERE Code=@Code";
                foreach (DataRow dr in drs)
                {
                    sqlCom = new SqlCommand(strSql, sqlConn, sqlTrain);
                    sqlParam = new SqlParameter();
        ...
        public void Delete(string sCode, out int iReturnValue, out string sMsg)
        {
            ...
            try
            {
                ...
                sqlCom.CommandText = "[JC_FactoryCode_Delete]";
