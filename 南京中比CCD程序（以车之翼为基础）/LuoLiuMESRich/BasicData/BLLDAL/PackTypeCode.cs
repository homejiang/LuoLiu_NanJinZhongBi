    ...
    public class PackTypeCode
    {
        ...
        public void Save(DataSet ds)
        {
            ...
            try
            {
                ...
                DataRow[] drs;
                #region 新增数据
                drs = ds.Tables["JC_PackTypeCode"].Select("", "", DataViewRowState.Added);
                strSql = @"INSERT INTO JC_PackTypeCode (Code,CodeName,AnotherName,Remark,Terminated) 
                        SELECT @Code,@CodeName,@AnotherName,@Remark,@Terminated";
                foreach (DataRow dr in drs)
                {
                    ...
                    sqlParam = new SqlParameter();
                    sqlParam.ParameterName = "@CodeName";
                    sqlParam.SqlDbType = SqlDbType.NVarChar;
                    sqlParam.Size = 50;
                    sqlParam.Value = dr["CodeName"];
                    sqlCom.Parameters.Add(sqlParam);
                    sqlParam = new SqlParameter();
                    sqlParam.ParameterName = "@AnotherName";
                    sqlParam.SqlDbType = SqlDbType.NVarChar;
                    sqlParam.Size = 50;
                    sqlParam.Value = dr["AnotherName"];
                    sqlCom.Parameters.Add(sqlParam);
                    sqlParam = new SqlParameter();
                ...
                #endregion
                #region 更新数据
                drs = ds.Tables["JC_PackTypeCode"].Select("", "", DataViewRowState.ModifiedCurrent);
                strSql = @"UPDATE JC_PackTypeCode SET Code=@Code,CodeName=@CodeName,AnotherName=@AnotherName,Remark=@Remark,Terminated=@Terminated  WHERE Code=@OriginalCode";
                foreach (DataRow dr in drs)
                {
                    sqlCom = new SqlCommand(strSql, sqlConn, sqlTrain);
                    sqlParam = new SqlParameter();
                ...
                #endregion
                #region 删除数据
                drs = ds.Tables["JC_PackTypeCode"].Select("", "", DataViewRowState.Deleted);
                strSql = @"DELETE FROM JC_PackTypeCode WHERE Code=@Code";
                foreach (DataRow dr in drs)
                {
                    sqlCom = new SqlCommand(strSql, sqlConn, sqlTrain);
                    sqlParam = new SqlParameter();
        ...
        public void Delete(string sCode, out int iReturnValue, out string sMsg)
        {
            ...
            try
            {
                ...
                sqlCom.CommandText = "[JC_PackTypeCode_Delete]";
