    ...
    public class ProcessMacs
    {
        ...
        public void Save(DataSet ds)
        {
            ...
            try
            {
                ...
                DataRow[] drs;
                #region 新增数据
                drs = ds.Tables["JC_ProcessMacs"].Select("", "", DataViewRowState.Added);
                strSql = @"INSERT INTO JC_ProcessMacs (Code,MacName,Address,Terminated,SortID,ProcessCode,Remark) 
                        SELECT @Code,@MacName,@Address,@Terminated,@SortID,@ProcessCode,@Remark";
                foreach (DataRow dr in drs)
                {
                    ...
                    sqlParam = new SqlParameter();
                    sqlParam.ParameterName = "@MacName";
                    sqlParam.SqlDbType = SqlDbType.NVarChar;
                    sqlParam.Size = 30;
                    sqlParam.Value = dr["MacName"];
                    sqlCom.Parameters.Add(sqlParam);
                    sqlParam = new SqlParameter();
                ...
                #endregion
                #region 更新数据
                drs = ds.Tables["JC_ProcessMacs"].Select("", "", DataViewRowState.ModifiedCurrent);
                strSql = @"UPDATE JC_ProcessMacs SET Code=@Code,MacName=@MacName,Address=@Address,Terminated=@Terminated,SortID=@SortID,ProcessCode=@ProcessCode,Remark=@Remark WHERE Code=@OriginalCode";
                foreach (DataRow dr in drs)
                {
                    sqlCom = new SqlCommand(strSql, sqlConn, sqlTrain);
                    sqlParam = new SqlParameter();
                ...
                #endregion
                #region 删除数据
                drs = ds.Tables["JC_ProcessMacs"].Select("", "", DataViewRowState.Deleted);
                strSql = @"DELETE FROM JC_ProcessMacs WHERE Code=@Code";
                foreach (DataRow dr in drs)
                {
                    sqlCom = new SqlCommand(strSql, sqlConn, sqlTrain);
                    sqlParam = new SqlParameter();
        ...
        public void Delete(string sCode, out int iReturnValue, out string sMsg)
        {
            ...
            try
            {
                ...
                sqlCom.CommandText = "[JC_ProcessMacs_Delete]";
