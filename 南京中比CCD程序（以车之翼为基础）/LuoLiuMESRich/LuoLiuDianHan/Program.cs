...
namespace LuoLiuDianHan
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Common.CurrentUserInfo.UserCode = "MES";
            Common.CurrentUserInfo.UserName = "MES系统";
            if (!Common.CommonDAL.ReadInifile()) return;
            if (Common.CommonDAL.MainProgramName == "" || string.Compare(Common.CommonDAL.MainProgramName, "LuoLiuDianHan.exe", true) == 0)
        ...
        static void StartPro()
        {
            ...
            if (!cfg.ReadConfig()) return;
            if (!Common.CommonFuns.OneInstance.IsFirst("hangzhouchenhenLuoLiuDianHanDevelopeBychenguiyang@##+" + Config.MacCode))
            {
                MessageBox.Show("当前机台的程序已经打开！");
                return;
            }
            if (Config.ProcessCode.Length == 0 || Config.StationCode.Length == 0 || Config.MacCode.Length == 0)
            {
            ...
            Application.Run(new frmDianHan());
        }
    }
    #region 读取版本信息
