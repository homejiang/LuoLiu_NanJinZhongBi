...
namespace LuoLiuMES.Boxing
{
    ...
    public class BoxPrinter
    {
        ...
        public string MyArg = string.Empty;
        public BoxPrinter()
        {
            ...
            if (BoxPrinter.PrinterMachineName.Length > 0)
                SetPrinterMachine(BoxPrinter.PrinterMachineName);
        }
        #region 公共函数
        public bool SetPrinterMachine(string sMachineName)
        {
            ...
            {
                this.PrintFinishedNoticeAsyn(this.MyTuoPanCode, false, string.Format("设置打印机出错{0}({1})", ex.Message, ex.Source), this.MyArg);
                return false;
            }
            return true;
        }
        ...
        {
            return Printing(sTuoPanCode, string.Empty);
        }
        public bool Printing(string sTuoPanCode,string sArg)
        {
            this.MyTuoPanCode = sTuoPanCode;
            this.MyArg = sArg;
            if (!this.SetPrinterMachine(BoxPrinter.PrinterMachineName))
                return false;
            try
            {
            ...
            {
                this.PrintFinishedNoticeAsyn(sTuoPanCode, false, string.Format("调用打印机出错，请检查打印机是否连接？{0}({1})", ex.Message, ex.Source), sArg);
                return false;
            }
            return true;
        }
        ...
        private void printDocument_PrintA4Page(object sender, PrintPageEventArgs e)
        {
            try
            {
                ...
                Bitmap map = this.CreateQRCode(this.MyTuoPanCode, BoxPrinter.QRSize);
                map.SetResolution(203F, 203F);
                Rectangle destRect = new Rectangle(BoxPrinter.QRSet_Left, BoxPrinter.QRSet_Top, QRSet_Rectangle, QRSet_Rectangle);
                ImageAttributes imageAttributes = new ImageAttributes();
                e.Graphics.DrawImage(map, destRect, 0, 0, QRSet_Inch, QRSet_Inch, GraphicsUnit.Inch, imageAttributes);
                string strCode = string.Empty;

                if (this.MyTuoPanCode.Length > BoxPrinter.WordsRN)
                {
                    int iStartWords = 0;
                    while (iStartWords < this.MyTuoPanCode.Length)
                    {
                        if (this.MyTuoPanCode.Length >= iStartWords + BoxPrinter.WordsRN)
                            strCode += this.MyTuoPanCode.Substring(iStartWords, BoxPrinter.WordsRN) + "\r\n";
                        else strCode += this.MyTuoPanCode.Substring(iStartWords) + "\r\n";
                        iStartWords += BoxPrinter.WordsRN;
                    }
                }
                else strCode = this.MyTuoPanCode;
                if (strCode.EndsWith("\r\n"))
                    strCode = strCode.Substring(0, strCode.Length - 2);
                e.Graphics.DrawString(strCode, evFont, System.Drawing.Brushes.Black, new Point(BoxPrinter.WordsSet_Left, BoxPrinter.WordsSet_Top));
                this.PrintFinishedNoticeAsyn(this.MyTuoPanCode, true, string.Empty,this.MyArg);
            }
            catch (Exception ex)
            {
                this.PrintFinishedNoticeAsyn(this.MyTuoPanCode, false, string.Format("Printing.Err:{0}({1})", ex.Message, ex.Source), this.MyArg);
            }
        }
        private void PrintFinishedNoticeAsyn(string sTuoPanCode, bool blSucessful, string sErr, string sArg)
        {
            
            if (this.PrintFinishedNotice != null)
                this.PrintFinishedNotice(sTuoPanCode, blSucessful, sErr, sArg);
        }
        private void CallPrintFinishedNotice(short iCaoIndex, string sTuoPanCode, bool blSucessful, string sErr, string sArg)
        {
            if (this.PrintFinishedNotice != null)
                this.PrintFinishedNotice(sTuoPanCode, blSucessful, sErr, sArg);
        }

        public delegate void PrintFinishedCallback(string sTuoPanCode, bool blSucessful, string sErr,string sArg);
        #endregion
    }
    #endregion
}
