    ...
    public class BoxType
    {
        ...
        public void Save(DataSet ds)
        {
            ...
            try
            {
                ...
                DataRow[] drs;
                #region 新增数据
                drs = ds.Tables["JC_BoxType"].Select("", "", DataViewRowState.Added);
                strSql = @"INSERT INTO JC_BoxType (Code,TypeName,Terminated,Qty,MyWeight) 
                    SELECT @Code,@TypeName,@Terminated,@Qty,@MyWeight";
                foreach (DataRow dr in drs)
                {
                    ...
                    sqlCom.Parameters.Add(sqlParam);
                    sqlParam = new SqlParameter();
                    sqlParam.ParameterName = "@MyWeight";
                    sqlParam.SqlDbType = SqlDbType.Decimal;
                    sqlParam.Size = 9;
                    sqlParam.Value = dr["MyWeight"];
                    sqlCom.Parameters.Add(sqlParam);
                ...
                #endregion
                #region 更新数据
                drs = ds.Tables["JC_BoxType"].Select("", "", DataViewRowState.ModifiedCurrent);
                strSql = @"UPDATE JC_BoxType SET Code=@Code,TypeName=@TypeName,Terminated=@Terminated,Qty=@Qty,MyWeight=@MyWeight WHERE Code=@OriginalCode";
                foreach (DataRow dr in drs)
                {
                    sqlCom = new SqlCommand(strSql, sqlConn, sqlTrain);
                    sqlParam = new SqlParameter();
                ...
                #endregion
                #region 删除数据
                drs = ds.Tables["JC_BoxType"].Select("", "", DataViewRowState.Deleted);
                strSql = @"DELETE FROM JC_BoxType WHERE Code=@Code";
                foreach (DataRow dr in drs)
                {
                    sqlCom = new SqlCommand(strSql, sqlConn, sqlTrain);
                    sqlParam = new SqlParameter();
        ...
        public void Delete(string sCode, out int iReturnValue, out string sMsg)
        {
            ...
            try
            {
                ...
                sqlCom.CommandText = "[JC_BoxType_Delete]";
